...
class CKGAnthropometryORM(Base):
    ...
    nutrition_status: Mapped[str | None] = mapped_column(String(80), nullable=True)
...
class CKGTTVORM(Base):
    ...
    blood_pressure_status: Mapped[str | None] = mapped_column(String(80), nullable=True)
...
class CKGVisionORM(Base):
    ...
    external_eye_exam: Mapped[str | None] = mapped_column(String(120), nullable=True)
    uses_glasses: Mapped[str | None] = mapped_column(String(20), nullable=True)
    vision_status: Mapped[str | None] = mapped_column(String(120), nullable=True)
    examined_by: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
...
class CKGGeneralScreeningORM(Base):
    ...
    anemia_status: Mapped[str | None] = mapped_column(String(80), nullable=True)
    blood_glucose: Mapped[float | None] = mapped_column(Float, nullable=True)
    diabetes_status: Mapped[str | None] = mapped_column(String(80), nullable=True)
    hbsag_result: Mapped[str | None] = mapped_column(String(80), nullable=True)
    anti_hcv_result: Mapped[str | None] = mapped_column(String(80), nullable=True)
    hearing_test: Mapped[str | None] = mapped_column(String(160), nullable=True)
    cerumen_impaction: Mapped[str | None] = mapped_column(String(20), nullable=True)
    fitness_result: Mapped[str | None] = mapped_column(String(160), nullable=True)
    frambusia_status: Mapped[str | None] = mapped_column(String(120), nullable=True)
    leprosy_status: Mapped[str | None] = mapped_column(String(120), nullable=True)
    scabies_status: Mapped[str | None] = mapped_column(String(120), nullable=True)
    referral_conclusion: Mapped[str | None] = mapped_column(String(40), nullable=True)
...
class StudentComplaintORM(Base):
    """Initial public complaint submitted on behalf of a student before UKS verification."""
